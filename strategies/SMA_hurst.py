...
class SMA_hurst(Strategy):
    ...
    def OnBarUpdate(self, data=Data, bar=Bar):
        ...
        # 实盘需要考虑tick的连续添加
        self.hurst.append(np.array(self.calcHurst2(self.vola[-30:-1])))   


        if len(self.hurst) > 5:
            mahurst = np.mean(self.hurst[-6:-2]) 
            ...
            if mahurst < 0.45 and self.PositionLong >0:
                self.Sell(self.O[-1], self.p_lots, '卖平')
            
            if mahurst < 0.45 and self.PositionShort >0:
                self.BuyToCover(self.O[-1], self.p_lots, '买平')

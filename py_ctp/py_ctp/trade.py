...
class CtpTrade():
    ...
    def _OnFrontConnected(self):
        threading.Thread(self.OnConnected, args=(self,)).start()
    ...
    def _OnFrontDisconnected(self, nReason):
        ...
        if nReason == 4097 or nReason == 4098:
            threading.Thread(self._reconnect).start()
        else:
            threading.Thread(self.OnDisConnected, args=(self, nReason)).start()
    ...
    def _OnRspUserLogin(self, pRspUserLogin: CThostFtdcRspUserLoginField(), pRspInfo: CThostFtdcRspInfoField, nRequestID: int, bIsLast: bool):
        ...
        if pRspInfo.getErrorID() == 0:
            ...
            threading.Thread(self._relogin).start()
        else:
            info = InfoField()
            info.ErrorID = pRspInfo.getErrorID()
            info.ErrorMsg = pRspInfo.getErrorMsg()
            threading.Thread(self.OnUserLogin, args=(self, info)).start()
    ...
    def _OnRspSettlementInfoConfirm(
        ...
        if not self.logined:
            time.sleep(0.5)
            """查询合约/持仓/权益"""
            threading.Thread(self._qry).start()  # 开启查询
    ...
    def _OnRtnOrder(self, pOrder: CThostFtdcOrderField):
        ...
        if not of:
            ...
            self.orders[id] = of
            threading.Thread(self.OnOrder, args=(self, of)).start()
            ...
            if of.StatusMsg.find('被拒绝') >= 0:
                ...
                threading.Thread(self.OnErrOrder, args=(self, of, info)).start()
            else:
                threading.Thread(self.OnCancel, args=(self, of)).start()
        else:
    ...
    def _OnRtnTrade(self, f):
        ...
        if tf.Offset == OffsetType.Open:
            ...
            if pf:  # 有可能出现无持仓的情况
                ...
                pf.Position -= tf.Volume
        threading.Thread(self._onRtn, args=(of, tf)).start()
    ...
    def _OnRspOrderAction(self, pInputOrderAction: CThostFtdcInputOrderActionField, pRspInfo: CThostFtdcRspInfoField, nRequestID: int, bIsLast: bool):
        ...
        if self.logined and id in self.orders:
            ...
            threading.Thread(self.OnErrCancel, args=(self, self.orders[id], info)).start()
    ...
    def ReqUserLogout(self):
        ...
        self.t.Release()
        threading.Thread(self.OnDisConnected, args=(self, 0)).start()
